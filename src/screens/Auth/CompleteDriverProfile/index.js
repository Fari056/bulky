...
const CompleteDriverProfile = ({ navigation, route }) => {
  ...
  const isValid = !Make || !Model || !Color || !Class || !Type || !Number || imageUrls.length === 0

  const save = async () => {
    if (!Make || !Model || !Color || !Class || !Type || !Number || imageUrls.length === 0) {
      console.error("All fields must be filled and at least one image must be uploaded.");
      ToastError('All fields must be filled and at least one image must be uploaded.')
      return;
    }
    setLoading(true)
    try {
